...
def convert_gif_to_webm(input_path, output_path):
    ...
    自动调整 CRF 以确保文件大小小于 256KB
    """
    
    # Telegram 限制: 256 KB. 为了安全起见，我们设定目标为 255 KB
    MAX_SIZE = 255 * 1024
    
    # 起始 CRF 30 (高质量)，最大 CRF 55 (低质量)
    # 每次重试增加 5
    current_crf = 30
    
    # 构建 FFmpeg 滤镜字符串
    # 1. format=rgba: 先将 GIF 转换为 RGBA 格式，防止缩放时丢失透明通道
    # 2. fps=30: 强制帧率
    # 3. scale: 缩放图片，使长边为512，保持纵横比
    # 4. pad: 将画布扩展为 512x512，居中，背景透明 (color=0x00000000)
    filter_complex = (
        "format=rgba,"
        "fps=30,"
    ...
    print(f"Failed: {os.path.basename(input_path)} could not be compressed under 256KB even with CRF 55.")
    return False
